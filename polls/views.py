...
AIRTABLE_API_KEY = "keyFnSbIK4yqtGSX1"
AIRTABLE_MOVIESTABLE_BASE_ID = "appjB6yuwC6MdLnct"
...
def create_movie(request):
    ...
    print(image_url)
    new_movie = AT.insert({'Name': title, 'Photos': image_url,
...
def search_form(request):
    ...
    movie_thingy = tmdb_search.movie(
        query=str(request.GET.get('query', '')) or ' ')
    movie_poster_urls = []
    movie_titles = []
    movies = []
    ...
    for movie in movie_thingy['results']:
        movies.append(movie)
        movie_poster_urls.append(
            'https://image.tmdb.org/t/p/w500' + str(movie.get('poster_path', '/')))
